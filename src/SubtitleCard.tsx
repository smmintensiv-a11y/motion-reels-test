import React from 'react';
import {SUBTITLE_TEMPLATE} from './hookTemplate.config';

export const SubtitleCard: React.FC<{text: string; opacity: number}> = ({text, opacity}) => {
	const {colors, plate, text: textStyle} = SUBTITLE_TEMPLATE;

	return (
		<div
			style={{
				position: 'absolute',
				left: plate.sideMargin,
				right: plate.sideMargin,
				bottom: plate.bottomSafeMargin,
				opacity,
				display: 'flex',
				alignItems: 'stretch',
				borderRadius: plate.borderRadius,
				background: colors.plateBackground,
				backdropFilter: 'blur(10px)',
				boxShadow: '0 18px 46px rgba(0,0,0,0.45)',
				overflow: 'hidden',
			}}
		>
			{/* Red accent line, matching the hook card */}
			<div
				style={{
					width: plate.accentWidth,
					alignSelf: 'stretch',
					background: `linear-gradient(180deg, ${colors.accentTop} 0%, ${colors.accentBottom} 100%)`,
					boxShadow: '0 0 14px rgba(255,40,50,0.6)',
				}}
			/>

			{/* Text block */}
			<div
				style={{
					padding: `${plate.paddingTop}px ${plate.paddingRight}px ${plate.paddingBottom}px ${plate.paddingLeft}px`,
					fontFamily: textStyle.fontFamily,
					fontWeight: textStyle.fontWeight,
					color: colors.textColor,
					fontSize: textStyle.fontSize,
					lineHeight: textStyle.lineHeight,
					letterSpacing: textStyle.letterSpacing,
					textShadow: '0 2px 10px rgba(0,0,0,0.55)',
				}}
			>
				{text}
			</div>
		</div>
	);
};
